...
function KpiCard({ label, value, sub, icon, iconBg, valueColor, trend }: KpiCardProps) {
  return (
    <div className="rounded-2xl border bg-card p-5 flex flex-col gap-3 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 group">
...
export function HeroMetricsSection({ metrics, teamCount }: Props) {
  const cards: KpiCardProps[] = [
    ...
    },
    {
      ...
      trend: metrics.overdueTasks > 0
        ? { label: "2", positive: false }
        : undefined,
    },
    {
      label: "متوسط عبء العمل",
