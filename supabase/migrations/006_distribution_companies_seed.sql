...
-- Sync real company list — UPSERT only, no data loss
--
-- Safe to run in production:
--   • Existing companies are updated (name, type, sort_order).
--   • No rows are deleted — operational data and FK relations
--     in distribution_requests are fully preserved.
--   • Running this migration more than once has no side-effects.
--
-- Dev-only cleanup (run manually, never in production):
--   TRUNCATE distribution_companies CASCADE;
-- ============================================================
...
-- UPSERT: insert new companies or update metadata of existing ones.
-- created_at is intentionally excluded from the DO UPDATE clause
-- so it is never overwritten for companies that already exist.
...
ON CONFLICT (name) DO UPDATE SET
  ...
  -- created_at is NOT updated — preserves original insertion timestamp
